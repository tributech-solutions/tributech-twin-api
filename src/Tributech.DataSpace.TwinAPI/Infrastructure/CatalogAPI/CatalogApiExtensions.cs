...
namespace Tributech.DataSpace.TwinAPI.Infrastructure.CatalogAPI {
	public static class CatalogApiExtensions {
		...
		public static IServiceCollection AddCatalogAPIClient(this IServiceCollection services, Action<ClientAuthOptions> configure) {
			...
			//services.AddHttpClient<CatalogAPIClient>((sp, httpClient) => {
			//	httpClient.BaseAddress = new Uri(sp.GetRequiredService<IOptions<CatalogAPIOptions>>().Value.ApiEndpoint);
			//});

			services.AddHttpClientWithClientCredentialsAuth<CatalogAPIClient, CatalogAPIClient>(configure, nameof(CatalogAPIOptions))
